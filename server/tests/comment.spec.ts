...
const getCommentsByUserSpy = jest.spyOn(util, 'getCommentsByUser');
...
describe('GET /getComment', () => {
  it('should retrieve comments by username', async () => {
    ...
    getCommentsByUserSpy.mockResolvedValueOnce(mockComments);

    const response = await supertest(app).get('/comment/getComment').query({ username });
  ...
  it('should return an empty array if no comments are found for the username', async () => {
    const username = 'nonexistentuser';

    getCommentsByUserSpy.mockResolvedValueOnce([]);
  ...
  it('should return a 500 error if there is a database error', async () => {
    ...
    getCommentsByUserSpy.mockRejectedValueOnce(new Error('Database error'));
