...
import { Comment, AddCommentRequest, FakeSOSocket, FindUserByUsername } from '../types';
import {
  addComment,
  getCommentsByUser,
  populateDocument,
...
const commentController = (socket: FakeSOSocket) => {
   ...
   * @returns A Promise that resolves to void.
   */
  const getCommentsByUsername = async (req: FindUserByUsername, res: Response): Promise<void> => {
    const { username } = req.query;
    try {
      const clist: Comment[] = await getCommentsByUser(username);
      res.json(clist);
    } catch (err: unknown) {
  ...
  router.get('/getComment', getCommentsByUsername);
